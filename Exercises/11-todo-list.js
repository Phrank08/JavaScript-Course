...
function renderArray() {
  ...
  for(i = 0; i < todoArray.length; i++) {   // looping through the empty array (todoArray)
    const todoObject = todoArray[i];    // accessing every data in the array
    ...
    const html = `
      ...
      <div>${dueDate}</div>
        ...
        <button onclick="
          ...
          renderArray();
        " class="delete-btn">Delete</button>`   // this creates a container that will display the data on the page in grid form and also a delete button that deletes the todo
